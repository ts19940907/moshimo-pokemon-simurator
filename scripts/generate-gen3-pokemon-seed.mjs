/**
 * Generate Gen3 (Hoenn, dex 252–386) pokemon JSON + additive SQL seed.
 * Does not truncate existing Gen1/Gen2 rows — INSERT only.
 *
 * Usage: node scripts/generate-gen3-pokemon-seed.mjs
 */
...
/** Hoenn legendaries (box-restricted style). */
const RESTRICTED = new Set([
  377, 378, 379, 380, 381, 382, 383, 384,
]);
const MYTHICAL = new Set([385, 386]);
...
async function evolvesFurtherInHoenn(speciesUrl, dexNo) {
...
async function main() {
  ...
  for (let dex = 252; dex <= 386; dex += 1) {
    process.stdout.write(`\rfetch ${dex}/386`);
    ...
    // PokeAPI returns modern typings (Fairy). Strip Fairy for Gen3-era rows.
    let type1 = TYPE_NAME_TO_ID[types[0]?.type.name] ?? 1;
    let type2 =
    ...
    const evolves = await evolvesFurtherInHoenn(pokemon.species.url, dex);
    let category = "normal";
  ...
  fs.writeFileSync(
    path.join(root, "src/data/gen3-pokemon.json"),
  ...
  fs.writeFileSync(path.join(seedDir, "gen3_pokemon.sql"), `${pokemonSql}\n`);
  fs.writeFileSync(
    path.join(seedDir, "gen3_pokemon_all.sql"),
    `${abilitySql}\n${pokemonSql}\n`,
  );
  console.log(
    `wrote src/data/gen3-pokemon.json, supabase/seed/gen3_pokemon.sql, gen3_pokemon_all.sql (${abilities.length} abilities)`,
  );
}
